...
- ✅ **第四条卖点引入时间轴动画符号并升级为 steps 步进式运动（始终居中有一个节点，严格对齐 Figma Node 13801:167079）**（已完成）：
  - **精准还原 Figma 时间轴矢量规格与双向几何对称**：
    ...
    - **中心绝对对齐与双向镜像对称**：容器总宽 `64px`，中线位于 `32.00px`；节点宽 `8px`（半径 `4px`），中心定位要求左边缘位于 `28.00px`。将流容器 `.storyline-stream` 的起点定位精确重定为 `left: -8.92px`，使第 3 个节点左边缘精准落于 `-8.92 + 2 × 18.46 = 28.00px`，圆心稳稳锁死在 `32.00px` 正中心；左右两侧节点边距同为 `9.54px`，达成亚像素级双向镜像对称；
  - **Steps 步进式节奏运动（Dwell & Snappy Step）**：
    - 废弃原先持续匀速漂移（`linear 2.2s`），改为周期 `1.6s` 的**步进式卡位节奏（`storyline-step-flow`）**；
    - **84% 时间（1.34s）稳稳静止停顿**：`0% ~ 72%`（1.15s）与 `88% ~ 100%`（0.19s）处于静止驻留态，**始终在视口正中呈现一个沉稳的居中节点**；
    - **16% 时间（0.25s）利落步进交替**：在 `72% ~ 88%`（256ms）区间，采用机械快门阻尼曲线 `cubic-bezier(0.25, 1, 0.4, 1)` 快速横向步进 `18.46px`（恰好一个完整节距），下一个节点如精密钟表齿轮卡位般精准滑入正中并锁死；
    - 循环衔接无任何视觉跳变或突兀感，完美兼具叙事隐喻（里程碑事件步进）与界面高级感。
