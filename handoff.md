...
- ✅ **第四条卖点引入时间轴动画符号并升级为 steps 步进式运动（14px 节点、始终居中、逆向流转、舒缓节奏）**（已完成）：
  - **14px 节点规格与双向几何中心对称**：
    - 节点直径调整为 **`14px × 14px`**（`.storyline-node`，`border: 2px solid #004F2B; background: #8EB876;`），显著提升在文字行内的辨识度与饱满度；
    - 外层容器相应微调为 `height: 16px; width: 64px;`，保障 14px 圆环及其 2px 边框完全无任何裁切；
    - 导轨底线（`.storyline-rail`）：`height: 2px; background-color: #004F2B; opacity: 0.2;`，穿过所有节点赤道正中心；
    - **中心绝对锁定与严格整数几何网格**：
      - 容器总宽 `64px`，中心位于 `32.00px`；节点宽 `14px`，居中时左边缘精确位于 `25.00px`（$32 - 7 = 25$）；
      - 节点间距设为 `gap: 8px`，节距 $P = 14 + 8 = 22px$；
      - 流容器 `.storyline-stream` 的起点定位精确设为 **`left: -19px`**（$25 - 2 \times 22 = -19$）：静止状态下第 3 个节点左边缘精确落在 `25.00px`（圆心位于 `32.00px` 正中）；
      - 左右可视节点中心分别位于 `10px` 与 `54px`，外侧左右留白均为严格相等的 `3px`，达成亚像素级双向镜像对称；
  - **逆向步进与舒缓停顿节奏（Slowed Reverse Steps）**：
    - **逆向向左步进（`0px ➔ -22px`）**：顺应时间流自右向左推进（新事件从右侧未来涌入、向左侧历史推进），位移设为 `-22px`；
    - **节奏明显放缓（周期 2.8s）**：
