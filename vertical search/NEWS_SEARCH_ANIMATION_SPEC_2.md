...
- **动画逻辑**：
  ...
  - **静态规格与绝对对称**：容器 `64px × 10px`，导轨高 `2px`（深色 `#242D29`，`opacity: 0.2`）；节点外径 `10px`（内芯 `6px`，外描边 `2px #242D29`，底色 `#8EB876`）；流容器起点 `left: -9.92px`，静止时第 3 节点圆心精确锁定在容器 `32px` 正中心，左右可视节点亚像素级对称。
